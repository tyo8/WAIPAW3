...
input_eid="/scratch/tyoeasley/WAPIAW3/subject_lists/example.txt"

# (output) filepath in which to store list of subject filepaths
output_fpath="/scratch/tyoeasley/WAPIAW3/brainrep_data/ICA_reps/subj_lists/example.txt"
...
# append a data filepath to ${output_fpath} for every eid in ${input_eid}
for eid in $(cat ${input_eid});
do
	# creates eid-string substitution input to correctly fill out the given filepath pattern
	inp_str=$( printf "%0.s ${eid}" $(seq 1 ${n_subs}) )
