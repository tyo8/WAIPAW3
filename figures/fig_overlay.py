...
brainreplist = ['Demo','ICA150']
featurelist1 = ['Social']
#featurelist2 = ['NetMats', 'spatialNMs']
featurelist2 = ['Amplitudes', 'NetMats','partial_NMs']
#featurelist2 = ['Volumes', 'Surface']

# create a dataset which represents the concatenated
# contents of a set of results files.
file_list = [file for file in os.listdir(indir) if file.endswith('.csv') and file.startswith('metrics') and 'age' not in file]
vals = pd.DataFrame()
for i in file_list:
...
print(vals)
#vals.loc[:, 'accuracy'] = vals['accuracy'].astype('float')
...
def plopiwaw_swarm(feats1, feats2, vals=vals, cohort='validation'):
    ...
    subgroup2['accuracy'] = subgroup2['accuracy'].astype(float)
    ...
    for i in feats1:
        for j in feats2:
            ...
            for k, group in enumerate(subgroup1['ptcode'].unique()):
                n = subgroup1[subgroup1['ptcode'] == group]['size'].max()
                plt.text(k, y_text_position,n, ha='center', va='bottom',fontsize=14)
