...
X_train, X_test, Y_train, Y_test = get_input_data(subj_list_fpath, datapath_type, test_prop=0.1, seed_num=0, patient_eid_dir=patient_eid_dirs, verbose=True)
Y_all,label_factor = _pull_group_labels(subj_list_fpath, patient_eid_dirs)

clf = OneVsRestClassifier(RandomForestClassifier(n_estimators = 10,criterion = "gini",random_state = 42)).fit(X_train,Y_train)
...
labels = ["".join("c" + str(i)) for i in range(0, 18)]
...
plt.savefig('/home/l.lexi/confusion.png')
